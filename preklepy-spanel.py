#zkouška rychlejšího opravování
#NEFUNGUJE protože nefunguje ani základní myšlenka a to ta že můžu mít spuštěných více funkcí najednou
...
if len(sys.argv) == 4:
  ...
  print(
    "napiš: kromě python opravit.py napiš co chceš opravit, kam to chceš nahrát a od 1 do 5 (5 nejodbornější)jak moc odborný text necháváš přeložit.")
  exit()

odbornost = int(odbornost)
if odbornost > 5:
  odbornost = 5  # volbu čísel nemám na ničem založenou, přišlo mi to ale jako zajímavý prvek
elif odbornost < 1:
  odbornost = 1  # kdyby bylo zadáno větší či menší číslo aby kód nevyhazoval blbosti
...
def najdi_zvlastni_znaky():  # najde zvláštní znaky vyndá je ze seznamu a přířadí k nim číslo místa, kde se nacházeli (v textu, ve slově(poslední první jinak bby mohl nastat problém s změnou indexu po smazání či přidání písmen))
  zvlastni_znaky_text = []
...
def rozloztext():  # zrychlit kontrolování tím, že text rozdělím na více částí po 15 slovech, které pak kontroluji součastně, na poslední
  seznam = []
  p = 0
  pp = 0
  ppp = 0
...
def cekuj(txt):
  ...
  for slovo in txt:
    if slovo in slovnik:
      ...
      txt[p] = podobne[lpp]   #txt[p].replace(text[p], podobne[lpp])
  ...
  print(txt)
  pripravtext[poradi] = txt


def zvlast_vracec():
  global text
  #print(str(text) + "první")
  text = []
  for kus in pripravtext:
    for slovo in kus:
      text.append(slovo)
  print(str(pripravtext) + "pripravtext")
...
zvlastni_znaky = najdi_zvlastni_znaky()

load_vocabulary(dictionary_path)
rozloztext()

for x in pripravtext:
  cekuj(x)
